...
from gill.config import HF_CACHE_DIR
...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="Number of [IMG] tokens to use.",
    )
    parser.add_argument(
        "--num-clip-tokens",
        default=77,
    ...
    parser.add_argument(
        ...
        "--dataset",
        metavar="DATASET",
        help="Delimited list of datasets:" + " | ".join(datasets),
        default="cc3.1m",
    ...
    parser.add_argument(
        ...
        default="cc3.1m",
        type=lambda s: [x for x in s.split(",")],
        help="Validation dataset: " + " | ".join(datasets) + " (default: cc3.1m)",
    )
    parser.add_argument(
        "--dataset-dir",
        default="datasets",
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="Number of steps to warm up lr.",
    )
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="Number of steps before decaying lr.",
    )
    parser.add_argument(
        "--lr_schedule_gamma",
        default=0.1,
        type=float,
        metavar="N",
        help="Decay parameter for learning rate scheduler.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--grad-clip", default=1.0, type=float, help="gradient clipping amount"
    )
    ...
    parser.add_argument(
        "--cap-loss-scale", type=float, default=1.0, help="Scale on captioning loss."
    )
    parser.add_argument(
        "--ret-loss-scale", type=float, default=1.0, help="Scale on retrieval loss."
    )
    parser.add_argument(
        "--gen-loss-scale", type=float, default=1.0, help="Scale on retrieval loss."
    )

    parser.add_argument(
        "--concat-captions-prob",
        type=float,
        default=0.5,
    ...
    parser.add_argument(
        "--image-size", default=224, type=int, metavar="N", help="Size of images."
    )
    parser.add_argument(
        "--ret-emb-dim",
        default=256,
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="Embedding dimension for generation.",
    )
    ...
    parser.add_argument(
        "--text-fc-mode",
        default="gill_mapper",
        choices=text_fc_modes,
        help="What kind of translation mapping to use.",
    )
    parser.add_argument(
        "--ret-text-fc-mode",
        default="linear",
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="Maximum length to truncate captions / generations to.",
    )
    parser.add_argument(
        "--n-visual-tokens",
        default=4,
        type=int,
        metavar="N",
        help="Number of visual tokens to use for the Frozen model.",
    )

    parser.add_argument(
        "--beta1", default=0.9, type=float, metavar="M", help="beta1 for Adam"
    )
    parser.add_argument(
        "--beta2", default=0.95, type=float, metavar="M", help="beta2 for Adam"
    )
    parser.add_argument(
        "--wd",
        "--weight-decay",
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    assert (
        args.num_tokens == 0
        or "gill_mapper" in model_args.text_fc_mode
        or (
            args.num_tokens * args.gen_emb_dim == args.num_clip_tokens * 768
            or args.num_tokens * args.gen_emb_dim == args.num_clip_tokens * 1024
        )
    ), f"{args.num_tokens} * {args.gen_emb_dim} != {args.num_clip_tokens} * 768 (or 1024)"

    tokenizer = AutoTokenizer.from_pretrained(
        args.opt_version, use_fast=False, cache_dir=HF_CACHE_DIR
    )
    ...
    writer.add_scalar(
        "params/total", total_trainable_params + total_nontrainable_params, 0
    )
    ...
    if not torch.cuda.is_available():
        ...
        if args.gpu is not None:
            ...
            args.val_batch_size = int(
                (args.val_batch_size or args.batch_size) / ngpus_per_node
            )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(
            train_loader, model, tokenizer, criterion, optimizer, epoch, scheduler, args
        )
        ...
        if not args.multiprocessing_distributed or (
            ...
            utils.save_checkpoint(
                ...
                is_best,
                os.path.join(args.log_dir, "ckpt"),
            )
    ...
    for i, (
        ...
        for model_mode in model_modes:
            ...
            if model_mode == "retrieval":
                ...
                tgt_tokens, token_len = (
                    ret_tokens,
                    ret_caption_len,
                )  # For captioning, it doesn't matter.
            ...
            if model_mode == "retrieval":
                ...
                caption_acc1, caption_acc5 = losses_utils.contrastive_acc(
                    logits_per_text, topk=(1, 5)
                )
                image_acc1, image_acc5 = losses_utils.contrastive_acc(
                ...
                if args.num_tokens != 0 and args.num_clip_tokens != args.num_tokens:
                    ...
                    last_embedding = last_embedding.reshape(
                        (last_embedding.shape[0], seq_len, -1)
                    )
                    assert last_embedding.shape == clip_emb.shape, (
                        last_embedding.shape == clip_emb.shape
                    )
        ...
        # Update weights
        if ((i + 1) % args.grad_accumulation_steps == 0) or (
            i == args.steps_per_epoch - 1
        ):
        ...
        with torch.no_grad():
            ...
            frozen_norm = torch.norm(
                model.module.model.input_embeddings.weight[: -args.num_tokens, :], dim=1
            ).mean(0)
            for ret_idx in args.retrieval_token_idx:
                trainable_weight = model.module.model.input_embeddings.weight[
                    ret_idx, :
                ]
                model.module.model.input_embeddings.weight[ret_idx, :].div_(
                    trainable_weight.norm(dim=-1) / frozen_norm
                )

            # Log norms to Tensorboard.
            embedding_norm = torch.norm(
                model.module.model.input_embeddings.weight, dim=1
            ).mean()
            ret_embedding_norm = torch.norm(
                model.module.model.input_embeddings.weight[args.retrieval_token_idx, :],
                dim=-1,
            ).mean()
        ...
        if actual_step == 1 or (i + 1) % args.print_freq == 0:
            print(
                "First 5 values of first 3 tokens of embedding matrix:",
                model.module.model.input_embeddings.weight.data[:3, :5],
            )
            if args.num_tokens > 0:
                print(
                    "First 5 values of [GEN0] token embeddings:",
                    model.module.model.input_embeddings.weight.data[
                        args.gen_token_idx[0], :5
                    ],
                )
                print(
                    f"First 5 values of [GEN{args.num_tokens-1}] token embeddings:",
                    model.module.model.input_embeddings.weight.data[
                        args.gen_token_idx[-1], :5
                    ],
                )
            print(
                "First 5 values of first [IMG0] token embeddings:",
                model.module.model.input_embeddings.weight.data[
                    args.retrieval_token_idx[0], :5
                ],
            )
            print(
                f"First 5 values of first [IMG{args.num_tokens-1}] token embeddings:",
                model.module.model.input_embeddings.weight.data[
                    args.retrieval_token_idx[-1], :5
                ],
            )
            ...
            writer.add_scalar(
                "train/embmat_all_norm", embedding_norm.item(), actual_step
            )
            writer.add_scalar(
                "train/embmat_ret_norm", ret_embedding_norm.item(), actual_step
            )
            writer.add_scalar(
                "train/vis_emb_norm_cap", cap_vis_emb_norm.avg, actual_step
            )
            writer.add_scalar(
                "train/vis_emb_norm_ret", ret_vis_emb_norm.avg, actual_step
            )
            writer.add_scalar("train/text_emb_norm", inp_emb_norm.avg, actual_step)
            writer.add_scalar(
                "metrics/total_secs_per_batch", batch_time.avg, actual_step
            )
            writer.add_scalar(
                "metrics/total_secs_captioning", cap_time.avg, actual_step
            )
            ...
            if not args.multiprocessing_distributed or (
                ...
                if model_mode == "captioning":
                    ...
                    if (
                        args.concat_captions_prob > 0
                        and model_mode == "captioning"
                        and generated_cap_images.shape[0] != caption_images.shape[0]
                    ):
                        generated_cap_images = torch.cat(
                            [generated_cap_images, generated_cap_images], axis=0
                        )
                    ...
                    grid = torchvision.utils.make_grid(
                        display_images, nrow=int(max_images_to_show**0.5), padding=4
                    )
                ...
                t2i_images = torch.stack(
                    [
                        normalized_images[retrieved_image_idx[i], ...]
                        for i in range(len(retrieved_image_idx))
                    ],
                    axis=0,
                )
                t2i_images = torch.cat(
                    [t2i_images.float().cpu(), caption_images], axis=2
                )[:max_images_to_show]
                t2i_grid = torchvision.utils.make_grid(
                    t2i_images, nrow=int(max_images_to_show**0.5), padding=4
                )
                ...
                retrieved_text = torch.stack(
                    [
                        caption_images[retrieved_image_idx[i], ...]
                        for i in range(len(retrieved_text_idx))
                    ],
                    axis=0,
                )
                i2t_images = torch.cat(
                    [normalized_images.float().cpu(), retrieved_text], axis=2
                )[:max_images_to_show]
                i2t_grid = torchvision.utils.make_grid(
                    i2t_images, nrow=int(max_images_to_show**0.5), padding=4
                )
