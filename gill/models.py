...
class GILLArgs:
    freeze_lm: bool = True
    freeze_vm: bool = True
    opt_version: str = "facebook/opt-13b"
...
class GILLModel(nn.Module):
    ...
    def __init__(self, tokenizer, args: GILLArgs = GILLArgs()):
        ...
        for layer_idx in self.args.text_emb_layers:
            if (layer_idx == -1 or layer_idx == self.lm.config.num_hidden_layers) and (
                "bert" not in opt_version
            ):
    ...
    def get_visual_embs(
        ...
        if mode not in ["captioning", "retrieval", "generation"]:
            raise ValueError(
                f"mode should be one of ['captioning', 'retrieval', 'generation'], got {mode} instead."
            )
        ...
        if mode == "captioning":
            ...
            visual_embs = torch.zeros(
                (pixel_values.shape[0], 1, 768), device=pixel_values.device
            )
        else:
            raise NotImplementedError
        ...
        if mode == "captioning":
            ...
            for label in full_labels:
                for k, token in enumerate(label):
                    ...
                    if (
                        token
                        in [self.tokenizer.pad_token_id]
            ...
            if concat_captions:
                ...
                for i in range(batch_size // 2):
                    ...
                    assert torch.all(
                        second_labels[bos_idx] == self.tokenizer.bos_token_id
                    ), (second_labels, bos_idx, self.tokenizer.bos_token_id)

                    # Remove BOS token of the second caption.
                    second_labels = torch.cat(
                        [second_labels[:bos_idx], second_labels[bos_idx + 1 :]], axis=0
                    )
                    second_emb = torch.cat(
                        [second_emb[:bos_idx, :], second_emb[bos_idx + 1 :, :]], axis=0
                    )
            ...
            if concat_captions:
                ...
                for i in range(batch_size // 2):
                    ...
                    all_last_embedding_idx.append(
                        (
                            last_embedding_idx[first_idx],
                            first_emb.shape[0] + last_embedding_idx[second_idx],
                        )
                    )

                    if mode == "retrieval":
                        assert (
                            concat_labels[all_last_embedding_idx[-1][0]]
                            in self.retrieval_token_idx
                        ), (concat_labels, all_last_embedding_idx[-1][0])
                        assert (
                            concat_labels[all_last_embedding_idx[-1][1]]
                            in self.retrieval_token_idx
                        ), (concat_labels, all_last_embedding_idx[-1][1])
                    elif mode == "generation":
                        ...
                        for gen_i in range(len(self.gen_token_idx)):
                            assert (
                                concat_labels[all_last_embedding_idx[-1][0] - gen_i]
                                == self.gen_token_idx[-gen_i - 1]
                            ), (
                                concat_labels,
                                all_last_embedding_idx[-1][0] - gen_i,
                                self.gen_token_idx[-gen_i - 1],
                            )
                            assert (
                                concat_labels[all_last_embedding_idx[-1][1] - gen_i]
                                == self.gen_token_idx[-gen_i - 1]
                            ), (
                                concat_labels,
                                all_last_embedding_idx[-1][1] - gen_i,
                                self.gen_token_idx[-gen_i - 1],
                            )
        ...
        if mode in ["retrieval", "generation"]:
            ...
            if not concat_captions:
                for idx, fc_layer in zip(self.args.text_emb_layers, text_hidden_fcs):
                    input_hidden_state = torch.stack(
                        [
                            output.hidden_states[idx][
                                i,
                                last_embedding_idx[i]
                                - num_tokens
                                + 1 : last_embedding_idx[i]
                                + 1,
                                :,
                            ]
                    ...
                    hidden_states.append(
                        fc_layer(input_hidden_state, input_embedding)
                    )  # (N, seq_len, 2048)
            else:
                for idx, fc_layer in zip(self.args.text_emb_layers, text_hidden_fcs):
                    all_last_embedding = []
                    ...
                    for i in range(batch_size // 2):
                        first_last_embedding_idx, second_last_embedding_idx = (
                            all_last_embedding_idx[i]
                        )
                        first_last_embedding = output.hidden_states[idx][
                            i,
                            first_last_embedding_idx
                            - num_tokens
                            + 1 : first_last_embedding_idx
                            + 1,
                            :,
                        ]  # (N, D)
                        second_last_embedding = output.hidden_states[idx][
                            i,
                            second_last_embedding_idx
                            - num_tokens
                            + 1 : second_last_embedding_idx
                            + 1,
                            :,
                        ]  # (N, D)
                        ...
                        first_last_output_logit = output.logits[
                            i, first_last_embedding_idx - 1, :
                        ]  # (N, D)
                        second_last_output_logit = output.logits[
                            i, second_last_embedding_idx - 1, :
                        ]  # (N, D)
                    ...
                    hidden_states.append(
                        fc_layer(last_embedding, input_embedding)
                    )  # (N, seq_len, 2048)
            ...
            if not concat_captions:
                ...
                last_embedding = torch.stack(hidden_states, dim=-1).sum(
                    dim=-1
                )  # torch.stack([last_hidden_state[i, :, :] for i in range(batch_size)], axis=0)  # (N, T, D)
            ...
            if mode == "retrieval":
                ...
                last_embedding = last_embedding / last_embedding.norm(
                    dim=1, keepdim=True
                )
    ...
    def generate(
        ...
        with torch.no_grad():  # no tracking history
            ...
            for i in range(max_len):
                output = self.lm(
                    inputs_embeds=embeddings, use_cache=False, output_hidden_states=True
                )
                ...
                if (
                    (self.retrieval_token_idx or self.gen_token_idx)
                    and self.retrieval_token_idx[0] != -1
                    and self.gen_token_idx[0] != -1
                ):
                    if i < min_word_tokens:
                        ...
                        if ret_scale_factor > 1:
                            logits[:, self.retrieval_token_idx[0]] = (
                                logits[:, self.retrieval_token_idx[0]].abs()
                                * ret_scale_factor
                            )
                        if gen_scale_factor > 1:
                            logits[:, self.gen_token_idx[0]] = (
                                logits[:, self.gen_token_idx[0]].abs()
                                * gen_scale_factor
                            )
                ...
                if (
                    ...
                    and next_token.item() == self.retrieval_token_idx[0]
                ):
                    ...
                    next_token = (
                        torch.tensor(self.retrieval_token_idx)[None, :]
                        .long()
...
class GILL(nn.Module):
    def __init__(
        ...
        if decision_model_path is not None:
            ...
            self.decision_model.load_state_dict(
                mlp_checkpoint["state_dict"], strict=True
            )
        ...
        with torch.no_grad():
            for p in prompts:
                if type(p) == Image.Image:
                    ...
                    text_ids = self.model.tokenizer(
                        p, add_special_tokens=add_bos, return_tensors="pt"
            ...
            all_ret_idx = [
                ...
                for i, x in enumerate(
                    generated_ids[0, :] == self.model.retrieval_token_idx[0]
                )
                if x
            ][:max_num_rets]
            ...
            if len(all_ret_idx) == 0:
                ...
                for ret_idx in all_ret_idx:
                    assert (
                        generated_ids[0, ret_idx : ret_idx + self.model.num_tokens]
                        .cpu()
                        .detach()
                        .numpy()
                        .tolist()
                        == self.model.retrieval_token_idx
                    ), (
                        generated_ids[0, ret_idx : ret_idx + self.model.num_tokens],
                        self.model.retrieval_token_idx,
                    )
                    raw_emb = embeddings[
                        :, ret_idx : ret_idx + self.model.num_tokens, :
                    ]  # (1, 8, 4096)
                    ...
                    if self.emb_matrix is not None:
                        ...
                        ret_emb = self.model.ret_text_hidden_fcs[0](raw_emb, None)[
                            :, 0, :
                        ]  # (1, 256)
                        ...
                        for img_idx in top_image_idx:
                            ...
                            try:
                                ...
                                image_outputs["ret"].append(
                                    (img, "ret", scores[img_idx].item())
                                )
                        ...
                        if self.decision_model is not None:
                            ...
                            probs = (
                                decision_logits.softmax(dim=-1)
                                .cpu()
                                .float()
                                .numpy()
                                .tolist()
                            )
                            image_outputs["decision"] = [
                                self.idx2dec[decision_logits.argmax().item()]
                            ] + probs
                    else:
                    ...
                    gen_prefix = "".join(
                        [f"[IMG{i}]" for i in range(self.model.args.num_tokens)]
                    )
                    gen_prefx_ids = self.model.tokenizer(
                        gen_prefix, add_special_tokens=False, return_tensors="pt"
                    ).input_ids.to(self.model.logit_scale.device)
                    gen_prefix_embs = self.model.input_embeddings(
                        gen_prefx_ids
                    )  # (1, T, D)
                    gen_emb = self.model.gen_text_hidden_fcs[0](
                    ...
                    gen_emb = gen_emb.repeat(
                        self.num_gen_images, 1, 1
                    )  # (self.num_gen_images, 77, 768)
                    ...
                    if self.load_sd:
                        ...
                        for img in gen_images:
                            pixel_values = utils.get_pixel_values_for_model(
                                ...
                                img.resize((224, 224)).convert("RGB"),
                            )
                            pixel_values = pixel_values.to(
                                device=self.model.logit_scale.device,
                                dtype=self.model.logit_scale.dtype,
                        ...
                        if self.emb_matrix is not None:
                            ...
                            gen_visual_embs = self.model.get_visual_embs(
                                all_gen_pixels, mode="retrieval"
                            )  # (1, D)
                            gen_visual_embs = gen_visual_embs / gen_visual_embs.norm(
                                dim=-1, keepdim=True
                            )
                            gen_visual_embs = gen_visual_embs.type(
                                self.emb_matrix.dtype
                            )
                            ...
                            if self.num_gen_images > 1:
                                image_outputs["gen"] = [
                                    (gen_images[idx], gen_rank_scores[idx].item())
                                    for idx in sorted_score_idx
                                ]
                            else:
                                image_outputs["gen"] = [
                                    (gen_images[0], gen_rank_scores.item())
                                ]
                        else:
                    ...
                    return_outputs.append(
                        utils.truncate_caption(caption) + f" {gen_prefix}"
                    )
                    return_outputs.append(image_outputs)

        return return_outputs

    def get_log_likelihood_scores(self, prompts: List):
        """
...
def load_gill(
    model_dir: str,
    load_ret_embs: bool = True,
    decision_model_fn: str = "decision_model.pth.tar",
) -> GILL:
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        model_kwargs["opt_version"], use_fast=False, cache_dir=HF_CACHE_DIR
    )
    ...
    with torch.no_grad():
        if "share_ret_gen" in model_kwargs:
            assert model_kwargs[
                "share_ret_gen"
            ], "Model loading only supports share_ret_gen=True for now."
        model.model.input_embeddings.weight[-model_kwargs["num_tokens"] :, :].copy_(
